// api/_chrome.js
//
// Shared Netscape Composer chrome (titlebar -> menubar -> toolbar -> format-bar ->
// framed .edit-frame viewport -> status bar). ONE source of truth for the window
// shell, imported by BOTH api/gallery.js and api/page/[slug]/index.js so /p/ pages
// get the same full frame the gallery has.
//
// Parameterized so the gallery call stays byte-identical (its bits are the defaults)
// while /p/ overrides the frame contents (a sandboxed <iframe>), the status-bar
// cluster, the head (OG tags), extra CSS, and the body script.
//
// NOTE: the gallery-grid / .doc-content CSS still lives in this file (it rode along
// with renderChrome). /p/ ships it unused (~cheap, cached). Lifting it into a
// gallery-only style block is a safe future cleanup; kept here now to guarantee the
// gallery output is unchanged. Build-time pages (scripts/templates/page.html) and the
// homepage (public/index.html) still carry their own chrome copies — future consolidation.
...
// Gallery defaults — used when a caller doesn't override, so gallery.js's call
// (which passes only title/content/statusText) renders exactly as before.
const DEFAULT_STATUS_RIGHT =
...
const GALLERY_BODY_SCRIPT = `<script>
  // Upvote wiring (gallery grid). Optimistic increment; on server confirmation we
  // replace with the authoritative count. Button locks into a "voted" state.
